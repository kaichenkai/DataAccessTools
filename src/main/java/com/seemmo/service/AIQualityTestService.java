 ...
 *
 */
public class AIQualityTestService extends BaseService {
    //单例
    private AIQualityTestService() {
    }//私有化构造方法使得该类无法在外部通过new 进行实例化
    public static AIQualityTestService instance = new AIQualityTestService();//准备一个类属性，指向一个实例化对象。 因为是类属性，所以只有一个
...
//        argsMap.put(BusinessConstant.INSTESV_STATUS, instesvStatus);
        ...
        if (imageDataMode.compositeModeText.isSelected()) {
            ...
            if (!"".equals(recogImageIndex)) {
                ...
                logging.debug("识别图索引" + "\t\t" + recogImageIndex);
            }
            //1.4 合成图数据
            byte[] fileByte;
