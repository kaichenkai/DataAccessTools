 ...
 * AI预审字段匹配 panel 类
 */
public class AITrafficFieldPanel extends JPanel {
    //单例
    private AITrafficFieldPanel() {//私有化构造方法使得该类无法在外部通过new 进行实例化
        this.setLayout(null);
...
//                ConfigPanel.instance.getHeight() - BaseConstant.CONST550);//设置坐标, 尺寸
    ...
    public static AITrafficFieldPanel instance = new AITrafficFieldPanel();//准备一个类属性，指向一个实例化对象。 因为是类属性，所以只有一个
...
//    }
    ...
    public void addComponents(){
        ...
        manufacturer.defaultValue.setText("");//恢复默认值
        ...
        recordId.checkBoxValue.setSelected(false);//
        recordId.checkBoxValue.setEnabled(true);//允许编辑
        ...
        deviceCode.defaultValue.setEditable(true);//设置为 可编辑
        //恢复设备编码
        deviceCode.defaultValue.setText("");
        ...
        carDirect.defaultValue.setText("");//恢复默认值
        carDirect.defaultValue.setVisible(true);//设置为 可见
        ...
        illegalTime.checkBoxValue.setSelected(false);//
        illegalTime.checkBoxValue.setEnabled(true);//允许编辑
