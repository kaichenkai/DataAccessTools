 ...
 */
public class CarPlateType extends BaseField{
    //单例
    private CarPlateType(){
        //carPlateTypeLabel
        label = new JLabel("车牌类型代码");
        label.setFont(FontClass.font14);
        label.setBounds(BaseConstant.CONST20, BaseConstant.CONST400, BaseConstant.CONST100, BaseConstant.CONST30);
        //carPlateTypeIndex
        ...
        index.setBounds(BaseConstant.CONST140, BaseConstant.CONST400, BaseConstant.CONST120, BaseConstant.CONST30);
        //carPlateTypeDefaultValue
        ...
        defaultValue.setBounds(BaseConstant.CONST300, BaseConstant.CONST400, BaseConstant.CONST90, BaseConstant.CONST30);
    }//私有化构造方法使得该类无法在外部通过new 进行实例化
    public static CarPlateType instance;//准备一个类属性，指向一个实例化对象。 因为是类属性，所以只有一个
    //public static 方法，提供给调用者,创建一次
    public static CarPlateType createInstance(){
        if(null==instance){//第一次访问的时候，发现instance没有指向任何对象，这时实例化一个对象
            instance = new CarPlateType();
        }
        return instance;
    }
}
