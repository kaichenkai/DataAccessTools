 ...
 */
public class CarDirect extends BaseField{
    //单例
    private CarDirect(){
        //carDirectLabel
        label = new JLabel("行车方向代码");
    ...
    public static CarDirect instance;//准备一个类属性，指向一个实例化对象。 因为是类属性，所以只有一个
    //public static 方法，提供给调用者,创建一次
    public static CarDirect createInstance(){
        if(null==instance){//第一次访问的时候，发现instance没有指向任何对象，这时实例化一个对象
            instance = new CarDirect();
        }
        return instance;
    }
}
